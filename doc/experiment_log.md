...
Measured results and settled findings. The untested backlog (proposals, hypotheses, go/no-go
plans) lives in [ideas.md](ideas.md); this file is the record those ideas reference. Mirrors
pgx's `docs/jass_experiment_log.md` on the training side.

All arena results use the built-in **paired** (swapped-deal) t-test; mean_diff is per *pair*
(= 2 games) unless noted.
...
## Heuristic-prior PUCT — no model needed (pre-AZ experiment 1)

Experiment 1 of the pre-AZ bridge plan in [ideas.md](ideas.md), run to completion: validates
PUCT plumbing and measures hand-coded heuristics used as the *tree policy* prior.
...
## Auto-regressive who-has-card model — findings

Idea, implementation options, and revival condition live in [ideas.md](ideas.md). Measured
results:
